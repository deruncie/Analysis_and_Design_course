...
author: Forrest Li, Mitchell Feldmann and Daniel Runcie
output: 
  html_notebook:
    toc: yes
    toc_float: yes
...
```{r}
#install.packages(c('rmarkdown','learnr','devtools','Matrix','lme4','lmerTest','pbkrtest','multcomp','multcompView','car','emmeans','ggplot2')) # remove the `#` symbol to run once. Then add it back so it doesn't run again!
```
...
We won't use them much in this class, but loops are what make programming powerful.
The idea of a loop is to repeat the same command over and over again.

```{r}
# for each value from 1 to 10, repeat the code within the brackets.
# the value i will take on that value for the duration of that loop
for(i in 1:10) {
  print(i) # print the value of i in this loop
}
```

The following command draws 10 random numbers 100 times, each time saving the mean of the 10 numbers in a vector.

```{r}
means <- rep(0,100) # prepare a vector for us to store the results
for(i in 1:100){  # this for loop repeats the commands below between the brackets for each number between 1 and 100
...
Check to see that `means` contains all the values:
