...
Our data is stored in a comma-separated-values table, so we'll use the `read_csv()` function.
...
When you run the command, you will see various output describing what was loaded.
You should check this carefully to make sure that `R` loaded the table correctly.
It is good to have the file open in `Excel` and compare.
